...
- **KV cache** — per-session KV cache pre-warmed on connect so existing conversations respond without prefill delay
- **Smart titles** — conversation titles generated by the model after the first exchange
- **Token-aware context trimming** — oldest messages dropped to stay within the 8k token budget (Qwen 2.5 supports 32k)
- **Log rotation** — server logs capped at 5 × 5 MB, never fill disk
...
| `SYSTEM_PROMPT` | Default system prompt for all conversations | `You are a helpful assistant…` |
